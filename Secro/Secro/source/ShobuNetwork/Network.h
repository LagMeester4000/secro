...
private:
	...
	// This function is called when both clients states are in sync
	// The function should store this "last common state" to be able to rollback to it later
    void (*m_storeCallback)(void *data);

    // Restore state callback function
	// This function rolls back to the "last common state"
    void (*m_restoreCallback)(void *data);

    // callback which returns an integer used to check if the game's state has diverged
	// It does a hash check (I think)
